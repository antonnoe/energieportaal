...
export function AIAdviseur({ veld, waarde, context }: AIAdviseurProps) {
  ...
  return (
    <div className="relative inline-flex" ref={ref}>
      ...
      {open && (
        <div className="absolute z-50 bottom-full mb-1 left-0 w-72 bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-xs text-gray-700 leading-relaxed">
...
function buildContextPrompt(
  ...
  const lines: string[] = [
    ...
    `- Fase project: ${toolState.subsidieStage || '(niet ingevuld)'}`,
    toolState.subsidieStage === 'gestart' ? '⚠️ WERKEN ZIJN AL GESTART — de meeste subsidies (MPR, CEE, Éco-PTZ) moeten VÓÓR aanvang worden aangevraagd. Waarschuw de gebruiker hier expliciet over!' : '',
    `- Woning ouder dan 2 jaar: ${toolState.subsidieAgeGt2 || '(niet ingevuld)'}`,
    ``,
    `VRAAG VAN GEBRUIKER:`,
    `Veld/sectie: ${veld}`,
  ];
