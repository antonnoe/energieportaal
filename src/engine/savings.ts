...
export function berekenSavings(
  ...
  const rawBesparingEur = maatregelen.reduce((s, m) => s + m.besparingEurJaar, 0);
  // Cap: besparing kan nooit meer zijn dan 90% van huidige kosten
  const maxBesparing = Math.round(huidigResultaat.kostenTotaal * 0.9);
  const totaalBesparingEur = Math.min(rawBesparingEur, maxBesparing);
...
export function berekenCombinatie(
  ...
  const besparingKwh = huidigResultaat.totaalVerbruikKwh - nieuwResultaat.totaalVerbruikKwh;
  const rawBesparingEur = huidigResultaat.nettoKosten - nieuwResultaat.nettoKosten;
  // Cap: besparing kan nooit meer zijn dan 90% van huidige kosten
  const besparingEur = Math.min(rawBesparingEur, Math.round(huidigResultaat.kostenTotaal * 0.9));
